...
cre_dt <- ctree( Loan_Status1 ~ ., data = credit_Risk1_train, controls = ctree_control(mincriterion = .70, minsplit = 75))
plot(cre_dt)


cer_dt_predict <- predict( cre_dt, credit_Risk1_test , type = "response")
cer_dt_predict

cer_DT_DF  <- data.frame( cer_dt_predict , credit_Risk1_test$Loan_Status1)
predict_table <- table(cer_dt_predict, credit_Risk1_test$Loan_Status1)
predict_table

cer_DT_Accuracy <- sum(diag(predict_table))/ sum(predict_table) * 100
cer_DT_Accuracy
...
# Again plot dt
cre_dt <- ctree( Loan_Status1 ~ ., data = credit_Risk1_train1, controls = ctree_control(mincriterion = .70, minsplit = 75))
plot(cre_dt)
